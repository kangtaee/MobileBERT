...
import pandas as pd
import numpy as np
from dask.array import optimize, argmax
...
# 3. 텍스트를 토큰으로 나눔 (토큰화)
tokenizer = MobileBertTokenizer.from_pretrained('mobliebert_uncased', do_lower_case=True)
...
# 6. 모델 설정
model = MobileBertForSequenceClassification.from_pretrained('mobliebert_uncased', num_labels=2)
model.to(device)
